        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            ...
            <div class="flex flex-wrap items-center gap-3">
                <button wire:click="refreshLogs"
                    ...
                    <span wire:loading wire:target="refreshLogs">Refreshing…</span>
                </button>
                <button wire:click="downloadLogs"
                        wire:loading.attr="disabled"
                        wire:target="downloadLogs"
                        class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-green-500/20 hover:bg-green-500/30 text-green-200 text-sm font-semibold tracking-wide uppercase transition">
                    <svg wire:loading.remove wire:target="downloadLogs" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                    </svg>
                    <svg wire:loading wire:target="downloadLogs" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                    <span wire:loading.remove wire:target="downloadLogs">Download</span>
                    <span wire:loading wire:target="downloadLogs">Downloading…</span>
                </button>
                <button wire:click="clearLogs"
                        wire:loading.attr="disabled"
                        wire:target="clearLogs"
                        wire:confirm="Are you sure you want to clear all logs? This action cannot be undone."
                        class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-red-500/20 hover:bg-red-500/30 text-red-200 text-sm font-semibold tracking-wide uppercase transition">
                    <svg wire:loading.remove wire:target="clearLogs" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                    </svg>
                    <svg wire:loading wire:target="clearLogs" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
    ...
    <div class="p-6 space-y-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <?php if($source): ?>
                    <span class="text-xs text-gray-400 dark:text-gray-500">Source: <?php echo e(ucfirst($source)); ?></span>
                <?php endif; ?>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <select wire:model.live="lines"
                    ...
                    <?php $__currentLoopData = [100, 200, 300, 500, 800, 1000]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $option): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($option); ?>"><?php echo e($option); ?> lines</option>
        ...
        <?php else: ?>
            <div class="bg-gray-900 text-green-300 rounded-xl border border-gray-800 shadow-inner">
                <div class="px-4 py-3 border-b border-gray-800 flex items-center justify-between text-xs text-green-500 uppercase tracking-wide">
                    <span><?php echo e($logType === 'docker' ? 'Container Logs' : 'Laravel Application Log'); ?></span>
                    <span><?php echo e(now()->format('M d, Y • H:i:s')); ?></span>
                </div>
                <div class="h-[28rem] overflow-y-auto scrollbar-thin">
                    <pre class="p-4 whitespace-pre-wrap text-sm leading-relaxed selection:bg-emerald-500/30"><?php echo e($logs ?: 'No log output available.'); ?></pre>
                </div>
            </div>
        <?php endif; ?>
    </div>
</div>
<?php /**PATH /home/roshdy/Work/projects/DEVFLOW_PRO/resources/views/livewire/projects/project-logs.blade.php ENDPATH**/ ?>
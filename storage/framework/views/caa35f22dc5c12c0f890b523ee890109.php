<div>
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-green-100 mt-2">Manage Let's Encrypt SSL certificates for <?php echo e($server->name); ?></p>
                ...
                <div class="flex items-center gap-3">
                    <a href="<?php echo e(route('servers.show', $server)); ?>" class="px-4 py-2.5 bg-white/10 hover:bg-white/20 text-white rounded-xl transition-all duration-200 font-medium">
                        Back to Server
                    </a>
                </div>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 transition-all duration-200 hover:shadow-xl">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-green-600 dark:text-green-400 mt-2"><?php echo e($this->stats['active']); ?></p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 transition-all duration-200 hover:shadow-xl">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-yellow-600 dark:text-yellow-400 mt-2"><?php echo e($this->stats['expiring_soon']); ?></p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 transition-all duration-200 hover:shadow-xl">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-red-600 dark:text-red-400 mt-2"><?php echo e($this->stats['expired']); ?></p>
                </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden transition-colors">
        ...
        <?php if($this->certificates->count() > 0): ?>
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        <?php $__currentLoopData = $this->certificates; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $certificate): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="font-medium text-gray-900 dark:text-white"><?php echo e($certificate->domain_name); ?></span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium border <?php echo e($certificate->getStatusBadgeClass()); ?>">
                                        <?php echo e($certificate->getStatusLabel()); ?>

                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-sm text-gray-600 dark:text-gray-400 capitalize"><?php echo e($certificate->provider); ?></span>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-sm text-gray-600 dark:text-gray-400">
                                        <?php echo e($certificate->issued_at ? $certificate->issued_at->format('M d, Y') : '-'); ?>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        <span class="text-sm text-gray-600 dark:text-gray-400">
                                            <?php echo e($certificate->expires_at ? $certificate->expires_at->format('M d, Y') : '-'); ?>

                                        </span>
                                        <?php if($certificate->expires_at && !$certificate->isExpired()): ?>
                                            <span class="text-xs text-gray-500 dark:text-gray-500">
                                                (<?php echo e($certificate->daysUntilExpiry()); ?> days)
                                            </span>
                                        <?php endif; ?>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <button wire:click="toggleAutoRenew(<?php echo e($certificate->id); ?>)"
                                            class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors <?php echo e($certificate->auto_renew ? 'bg-green-600' : 'bg-gray-300 dark:bg-gray-600'); ?>">
                                        <span class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform <?php echo e($certificate->auto_renew ? 'translate-x-6' : 'translate-x-1'); ?>"></span>
                                    </button>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        <!-- Renew Button -->
                                        <button wire:click="renewCertificate(<?php echo e($certificate->id); ?>)"
                                                wire:confirm="Renew certificate for <?php echo e($certificate->domain_name); ?>?"
                                                wire:loading.attr="disabled"
                                        ...
                                        <!-- Revoke Button -->
                                        <button wire:click="revokeCertificate(<?php echo e($certificate->id); ?>)"
                                                wire:confirm="⚠️ REVOKE CERTIFICATE?\n\nThis will revoke the certificate for <?php echo e($certificate->domain_name); ?>.\nThis action cannot be undone."
                                                wire:loading.attr="disabled"
                                        ...
                                        <!-- Delete Button -->
                                        <button wire:click="deleteCertificate(<?php echo e($certificate->id); ?>)"
                                                wire:confirm="Delete certificate record for <?php echo e($certificate->domain_name); ?>?\n\nThis will only delete the record from the database, not the actual certificate files."
                                                wire:loading.attr="disabled"
                                                title="Delete Record"
    ...
    <div class="mt-8 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-900 rounded-2xl shadow-lg p-6 border border-blue-200 dark:border-gray-700">
        ...
        <div class="space-y-3 text-sm text-gray-700 dark:text-gray-300">
            ...
            <div class="flex items-start gap-3">
                ...
                <p><strong>Step 3:</strong> Make sure your domain points to your server's IP address (<?php echo e($server->ip_address); ?>)</p>
            </div>
            <div class="flex items-start gap-3">
                <svg class="w-5 h-5 text-green-600 dark:text-green-400 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                ...
                                <div class="flex gap-3">
                                    ...
                                    <div class="text-sm text-blue-800 dark:text-blue-200">
                                        ...
                                        <ul class="list-disc list-inside space-y-1">
                                            <li>Ensure your domain points to <?php echo e($server->ip_address); ?></li>
...
<?php /**PATH /home/roshdy/Work/projects/DEVFLOW_PRO/resources/views/livewire/servers/s-s-l-manager.blade.php ENDPATH**/ ?>
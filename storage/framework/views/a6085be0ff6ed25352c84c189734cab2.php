...
<div wire:key="docker-management-<?php echo e($project->id); ?>" class="bg-white dark:bg-gray-800 rounded-lg shadow">
    <div class="p-6 border-b border-gray-200 dark:border-gray-700 dark:border-gray-700">
        <div class="flex justify-between items-center">
            ...
            <button wire:click="$refresh" 
                ...
                <span wire:loading.remove class="flex items-center space-x-1">
    ...
    <div class="border-b border-gray-200 dark:border-gray-700 dark:border-gray-700">
        <nav class="flex px-6 -mb-px space-x-8">
            <button wire:click="switchTab('overview')" 
                    ...
                    <?php if($activeTab === 'overview'): ?> border-blue-500 text-blue-600 dark:text-blue-400 <?php else: ?> border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 hover:border-gray-300 dark:hover:border-gray-600 <?php endif; ?>">
                Overview
            </button>
            <button wire:click="switchTab('images')" 
                    class="py-4 px-1 border-b-2 font-medium text-sm transition-colors
                    <?php if($activeTab === 'images'): ?> border-blue-500 text-blue-600 dark:text-blue-400 <?php else: ?> border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 hover:border-gray-300 dark:hover:border-gray-600 <?php endif; ?>">
                Images (<?php echo e(count($images)); ?>)
            </button>
            <button wire:click="switchTab('logs')" 
                    class="py-4 px-1 border-b-2 font-medium text-sm transition-colors
                    <?php if($activeTab === 'logs'): ?> border-blue-500 text-blue-600 dark:text-blue-400 <?php else: ?> border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 hover:border-gray-300 dark:hover:border-gray-600 <?php endif; ?>">
                Container Logs
            </button>
        </nav>
    </div>
    ...
    <?php else: ?>
        <div class="relative">
            <?php if($loading): ?>
                <div class="absolute inset-0 bg-slate-900/70 backdrop-blur flex flex-col items-center justify-center gap-3 rounded-xl z-20">
                    <svg class="w-6 h-6 animate-spin text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                    <p class="text-xs font-semibold tracking-wide uppercase text-blue-200">Refreshing Docker telemetry…</p>
                </div>
            <?php endif; ?>
            ...
            <div class="space-y-6 animate-fadeIn">
                    ...
                    <?php if($containerInfo): ?>
                        <div class="space-y-3">
                            <div class="flex justify-between items-center">
                                ...
                                <code class="bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-300 px-3 py-1 rounded text-sm"><?php echo e($containerInfo['Names'] ?? $project->slug); ?></code>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-gray-700 dark:text-gray-300 font-medium">Status:</span>
                                <span class="px-3 py-1 rounded-full text-sm font-medium
                                    <?php if(isset($containerInfo['State']) && stripos($containerInfo['State'], 'running') !== false): ?> bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400
                            ...
                            <?php if(isset($containerInfo['Ports'])): ?>
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="text-gray-900 dark:text-white"><?php echo e($containerInfo['Ports']); ?></span>
                                </div>
                            <?php endif; ?>
                            <?php if(isset($containerInfo['Image'])): ?>
                                <div class="flex justify-between items-center">
                                    <span class="text-gray-700 dark:text-gray-300 font-medium">Image:</span>
                                    <code class="bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-300 px-3 py-1 rounded text-sm"><?php echo e($containerInfo['Image']); ?></code>
                                </div>
                            <?php endif; ?>
                        </div>
                            ...
                            <div class="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
                                <?php if(isset($containerStats['CPUPerc'])): ?>
                                    <div class="bg-white dark:bg-gray-800 rounded-lg p-4 border border-blue-200 dark:border-blue-700 transition-colors">
                                        ...
                                        <p class="text-lg font-bold text-gray-900 dark:text-white"><?php echo e($containerStats['CPUPerc']); ?></p>
                                    </div>
                                <?php endif; ?>
                                <?php if(isset($containerStats['MemPerc'])): ?>
                                    <div class="bg-white dark:bg-gray-800 rounded-lg p-4 border border-blue-200 dark:border-blue-700 transition-colors">
                                        <p class="text-xs text-gray-600 dark:text-gray-400 mb-1">Memory Usage</p>
                                        <p class="text-lg font-bold text-gray-900 dark:text-white"><?php echo e($containerStats['MemPerc']); ?></p>
                                    </div>
                                <?php endif; ?>
                                <?php if(isset($containerStats['NetIO'])): ?>
                                    <div class="bg-white dark:bg-gray-800 rounded-lg p-4 border border-blue-200 dark:border-blue-700 transition-colors">
                                        <p class="text-xs text-gray-600 dark:text-gray-400 mb-1">Network I/O</p>
                                        <p class="text-sm font-bold text-gray-900 dark:text-white"><?php echo e($containerStats['NetIO']); ?></p>
                                    </div>
                                <?php endif; ?>
                                <?php if(isset($containerStats['BlockIO'])): ?>
                                    <div class="bg-white dark:bg-gray-800 rounded-lg p-4 border border-blue-200 dark:border-blue-700 transition-colors">
                                        <p class="text-xs text-gray-600 dark:text-gray-400 mb-1">Disk I/O</p>
                                        <p class="text-sm font-bold text-gray-900 dark:text-white"><?php echo e($containerStats['BlockIO']); ?></p>
                                    </div>
                                <?php endif; ?>
                            </div>
                        <?php endif; ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 transition-colors">
                        ...
                        <p class="text-2xl font-bold text-gray-900 dark:text-white"><?php echo e(count($images)); ?></p>
                    ...
                    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 transition-colors">
                        ...
                        <p class="text-2xl font-bold">
                            <span class="px-3 py-1 rounded-full text-sm
                                <?php if($project->status === 'running'): ?> bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400
                                <?php elseif($project->status === 'stopped'): ?> bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300
                    ...
                    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 transition-colors">
                        ...
                        <p class="text-lg font-bold text-gray-900 dark:text-white"><?php echo e($project->server->name); ?></p>
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1"><?php echo e($project->server->ip_address); ?></p>
                    </div>
                </div>
            </div>
        <?php endif; ?>
        ...
        <?php if($activeTab === 'images'): ?>
            <div class="animate-fadeIn">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Docker Images for <?php echo e($project->name); ?></h3>
                    <button wire:click="buildImage" 
                            class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition-colors">
                        🔨 Build New Image
                    </button>
                                ...
                                <?php $__currentLoopData = $images; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $image): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <code class="text-sm text-gray-900 dark:text-white"><?php echo e($image['Repository'] ?? 'N/A'); ?></code>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="px-2 py-1 text-xs bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-400 rounded"><?php echo e($image['Tag'] ?? 'latest'); ?></span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <code class="text-xs text-gray-600 dark:text-gray-400"><?php echo e(substr($image['ID'] ?? '', 0, 12)); ?></code>
                                        </td>

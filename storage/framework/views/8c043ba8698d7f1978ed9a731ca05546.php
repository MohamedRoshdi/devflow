<div wire:poll.60s x-data="{ bulkDropdownOpen: false, servicesDropdownOpen: false }">
    ...
    <div class="relative mb-8 rounded-2xl bg-gradient-to-br from-blue-500 via-indigo-500 to-purple-500 dark:from-blue-600 dark:via-indigo-600 dark:to-purple-600 p-8 shadow-xl overflow-hidden">
        ...
        <div class="relative z-10 flex justify-between items-center">
            ...
            <div class="flex space-x-3">
                ...
                <a href="<?php echo e(route('servers.create')); ?>" class="bg-white/20 hover:bg-white/30 backdrop-blur-md text-white font-semibold px-6 py-3 rounded-lg transition-all duration-300 hover:scale-105 shadow-lg">
                    + Add Server
                </a>
            </div>
        </div>
    ...
    <?php if(count($selectedServers) > 0): ?>
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    <div class="flex items-center space-x-2">
                        ...
                        <span class="text-white font-semibold"><?php echo e(count($selectedServers)); ?> server(s) selected</span>
                    </div>
                    ...
                    <div class="relative" x-data="{ open: false }">
                        ...
                        <div x-show="open"
                            ...
                            <button wire:click="bulkReboot"
                                    wire:confirm="Are you sure you want to reboot <?php echo e(count($selectedServers)); ?> server(s)? All running services will be interrupted."
                                    @click="open = false"
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 p-6 mb-6">
        ...
        <!-- Tag Filter -->
        <?php if($allTags->count() > 0): ?>
            <div class="border-t border-gray-200 dark:border-gray-700 pt-4">
                <div class="flex items-center justify-between mb-3">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Filter by Tags</label>
                    <a href="<?php echo e(route('servers.tags')); ?>" class="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300">
                        Manage Tags
                    </a>
                </div>
                <div class="flex flex-wrap gap-2">
                    <?php $__currentLoopData = $allTags; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tag): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <button wire:click="toggleTagFilter(<?php echo e($tag->id); ?>)"
                                class="inline-flex items-center px-3 py-1.5 rounded-full text-sm font-medium transition-all duration-200
                                    <?php if(in_array($tag->id, $tagFilter)): ?>
                                        ring-2 ring-offset-2 dark:ring-offset-gray-800 shadow-lg scale-105
                                    <?php else: ?>
                                        hover:scale-105 opacity-75 hover:opacity-100
                                    <?php endif; ?>"
                                style="background-color: <?php echo e($tag->color); ?>20; color: <?php echo e($tag->color); ?>; <?php if(in_array($tag->id, $tagFilter)): ?> ring-color: <?php echo e($tag->color); ?>; <?php endif; ?>">
                            <span class="w-2 h-2 rounded-full mr-2" style="background-color: <?php echo e($tag->color); ?>"></span>
                            <?php echo e($tag->name); ?>

                            <span class="ml-1.5 text-xs opacity-75">(<?php echo e($tag->servers_count); ?>)</span>
                        </button>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
            <?php $__currentLoopData = $servers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $server): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl hover:shadow-2xl transform hover:-translate-y-1 transition-all duration-300 relative group overflow-hidden"
                     :class="{ 'ring-4 ring-blue-500': <?php echo e(in_array($server->id, $selectedServers) ? 'true' : 'false'); ?> }">
                    <!-- Gradient Border Top -->
                    <div class="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r
                        <?php if($server->status === 'online'): ?> from-green-500 to-emerald-600
                        <?php elseif($server->status === 'offline'): ?> from-red-500 to-red-600
                        <?php elseif($server->status === 'maintenance'): ?> from-yellow-500 to-orange-500
                        <?php else: ?> from-gray-400 to-gray-500
                        <?php endif; ?>">
                    </div>
                    ...
                    <div class="absolute top-4 left-4 z-10" onclick="event.stopPropagation()">
                        <label class="flex items-center cursor-pointer">
                            <input type="checkbox"
                                   ...
                                   wire:change="toggleServerSelection(<?php echo e($server->id); ?>)"
                                   class="w-5 h-5 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                        </label>
                    </div>

                    <div class="p-6 cursor-pointer" onclick="window.location='<?php echo e(route('servers.show', $server)); ?>'">
                        <div class="flex items-start justify-between mb-4">
                            <div class="flex-1 ml-8">
                                <a href="<?php echo e(route('servers.show', $server)); ?>" class="text-xl font-bold text-gray-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
                                    <?php echo e($server->name); ?>

                                </a>
                                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1 font-mono"><?php echo e($server->ip_address); ?></p>
                            </div>
                            <div class="p-2 rounded-lg
                                <?php if($server->status === 'online'): ?> bg-gradient-to-br from-green-500 to-emerald-600
                                <?php elseif($server->status === 'offline'): ?> bg-gradient-to-br from-red-500 to-red-600
                                <?php elseif($server->status === 'maintenance'): ?> bg-gradient-to-br from-yellow-500 to-orange-500
                                <?php else: ?> bg-gradient-to-br from-gray-400 to-gray-500
                                <?php endif; ?>">
                                <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="mb-4 flex flex-wrap items-center gap-2">
                            <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full
                                <?php if($server->status === 'online'): ?> bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400
                                <?php elseif($server->status === 'offline'): ?> bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400
                                <?php elseif($server->status === 'maintenance'): ?> bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400
                            ...
                            <?php if($server->tags->count() > 0): ?>
                                <?php $__currentLoopData = $server->tags; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tag): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <span class="inline-flex items-center px-2 py-1 text-xs font-medium rounded-full"
                                          style="background-color: <?php echo e($tag->color); ?>20; color: <?php echo e($tag->color); ?>;">
                                        <span class="w-1.5 h-1.5 rounded-full mr-1.5" style="background-color: <?php echo e($tag->color); ?>"></span>
                                        <?php echo e($tag->name); ?>
                        ...
                        <div class="space-y-3 mb-4">
                            <div class="flex items-center justify-between p-3 bg-gradient-to-br from-blue-500/10 to-indigo-500/10 dark:from-blue-500/20 dark:to-indigo-500/20 rounded-lg">
                                ...
                                <span class="text-sm font-bold text-gray-900 dark:text-white"><?php echo e($server->cpu_cores ?? 0); ?> Cores</span>
                            </div>
                            ...
                            <div class="flex items-center justify-between p-3 bg-gradient-to-br from-purple-500/10 to-pink-500/10 dark:from-purple-500/20 dark:to-pink-500/20 rounded-lg">
                                ...
                                <span class="text-sm font-bold text-gray-900 dark:text-white"><?php echo e($server->memory_gb ?? 0); ?> GB</span>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-between items-center pt-4 border-t border-gray-200 dark:border-gray-700">
                            <div class="flex space-x-3" onclick="event.stopPropagation()">
                                <button wire:click="pingServer(<?php echo e($server->id); ?>)"
                                        wire:loading.attr="disabled"
                                        wire:loading.class="opacity-50"
                                        wire:target="pingServer(<?php echo e($server->id); ?>)"
                                    ...
                                    <svg wire:loading.remove wire:target="pingServer(<?php echo e($server->id); ?>)" class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.111 16.404a5.5 5.5 0 017.778 0M12 20h.01m-7.08-7.071c3.904-3.905 10.236-3.905 14.14 0M1.394 9.393c5.857-5.857 15.355-5.857 21.213 0"></path>
                                    </svg>
                                    <svg wire:loading wire:target="pingServer(<?php echo e($server->id); ?>)" class="animate-spin w-4 h-4 mr-1" fill="none" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                    </svg>
                                    <span wire:loading.remove wire:target="pingServer(<?php echo e($server->id); ?>)">Ping</span>
                                    <span wire:loading wire:target="pingServer(<?php echo e($server->id); ?>)">Pinging...</span>
                                </button>
                                <button wire:click="rebootServer(<?php echo e($server->id); ?>)"
                                        wire:confirm="Are you sure you want to reboot this server? All running services will be interrupted."
                                        wire:loading.attr="disabled"
                                        wire:loading.class="opacity-50"
                                        wire:target="rebootServer(<?php echo e($server->id); ?>)"
                                    ...
                                    <svg wire:loading.remove wire:target="rebootServer(<?php echo e($server->id); ?>)" class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>
                                    </svg>
                                    <svg wire:loading wire:target="rebootServer(<?php echo e($server->id); ?>)" class="animate-spin w-4 h-4 mr-1" fill="none" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                    </svg>
                                    <span wire:loading.remove wire:target="rebootServer(<?php echo e($server->id); ?>)">Reboot</span>
                                    <span wire:loading wire:target="rebootServer(<?php echo e($server->id); ?>)">Rebooting...</span>
                                </button>
                                ...
                                <a href="<?php echo e(route('servers.edit', $server)); ?>"
                                ...
                                <?php endif; ?>
                                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('delete', $server)): ?>
                                <button wire:click="deleteServer(<?php echo e($server->id); ?>)"
                                        wire:confirm="Are you sure you want to delete this server?"
                                        wire:loading.attr="disabled"
                                        wire:loading.class="opacity-50"
                                        wire:target="deleteServer(<?php echo e($server->id); ?>)"
                                        class="text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 text-sm font-medium transition-colors">
                                    <span wire:loading.remove wire:target="deleteServer(<?php echo e($server->id); ?>)">Delete</span>
                                    <span wire:loading wire:target="deleteServer(<?php echo e($server->id); ?>)">Deleting...</span>
                                </button>
                                <?php endif; ?>
                            </div>
                        </div>
    ...
    <?php else: ?>
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl text-center py-12 transition-colors">
            ...
            <a href="<?php echo e(route('servers.create')); ?>" class="inline-block bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white font-semibold px-6 py-3 rounded-lg transition-all duration-300 hover:scale-105 shadow-lg">
                + Add Server
            </a>
        </div>
    <?php endif; ?>
    ...
    <?php if($showResultsModal && !empty($bulkActionResults)): ?>
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full">
                    ...
                    <div class="px-6 py-4 max-h-96 overflow-y-auto">
                        <div class="space-y-3">
                            <?php $__currentLoopData = $bulkActionResults; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $serverId => $result): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="flex items-start p-4 rounded-lg border
                                    ...
                                    <div class="ml-3 flex-1">
                                        ...
                                        <?php if(isset($result['latency_ms'])): ?>
                                            <p class="text-xs mt-1 text-gray-600 dark:text-gray-400">
                                                Latency: <?php echo e($result['latency_ms']); ?>ms
                                            </p>
                                        <?php endif; ?>
                                    </div>
                                </div>

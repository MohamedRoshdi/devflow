            ...
            <div class="flex h-20 items-center justify-between rounded-full bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl shadow-sm shadow-blue-500/5 mt-6 px-6">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <?php if(auth()->guard()->check()): ?>
                        <a href="<?php echo e(route('dashboard')); ?>" class="inline-flex items-center justify-center px-5 py-2.5 text-sm font-semibold text-white bg-gray-900 hover:bg-gray-800 dark:bg-white dark:text-gray-900 dark:hover:bg-gray-100 rounded-full transition-colors">
                            Open Dashboard
                        </a>
                    <?php else: ?>
                        <a href="<?php echo e(route('login')); ?>" class="inline-flex items-center justify-center px-5 py-2.5 text-sm font-semibold text-white bg-gray-900 hover:bg-gray-800 dark:bg-white dark:text-gray-900 dark:hover:bg-gray-100 rounded-full transition-colors">
                            Sign In
                        </a>
                    <?php endif; ?>
    ...
    <main class="flex-1 mt-28 sm:mt-32">
                    ...
                    <div>
                        <?php if($this->projects->count() > 0): ?>
                            <div class="inline-flex items-center space-x-3 rounded-full bg-white/10 px-5 py-2 text-sm font-medium text-white/80 ring-1 ring-white/20 backdrop-blur">
                                ...
                                <span><?php echo e($this->projects->count()); ?> <?php echo e($this->projects->count() === 1 ? 'Project' : 'Projects'); ?> Live Now</span>
                            </div>
                        <?php endif; ?>
                        ...
                        <div class="mt-10 flex flex-wrap gap-4">
                            <?php if(auth()->guard()->check()): ?>
                                <a href="<?php echo e(route('dashboard')); ?>" class="inline-flex items-center gap-2 rounded-full bg-white px-6 py-3 text-sm font-semibold text-slate-900 shadow-xl shadow-slate-900/20 transition hover:-translate-y-0.5 hover:shadow-2xl">
                            ...
                            <?php else: ?>
                                <a href="<?php echo e(route('login')); ?>" class="inline-flex items-center gap-2 rounded-full bg-white px-6 py-3 text-sm font-semibold text-slate-900 shadow-xl shadow-slate-900/20 transition hover:-translate-y-0.5 hover:shadow-2xl">
                                    Request Access
                                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"></path>
                                    </svg>
                        ...
                        <dl class="mt-12 grid gap-8 sm:grid-cols-3">
                            <div>
                                ...
                                <dd class="mt-2 text-3xl font-semibold text-white"><?php echo e($this->projects->count()); ?></dd>
                            </div>
                            <div>
        ...
        <section id="projects" class="py-24 bg-white dark:bg-slate-900">
            <div class="mx-auto w-full max-w-[1560px] px-6 md:px-10 lg:px-16">
                ...
                <div class="mt-12 flex flex-col sm:flex-row gap-4 items-center justify-center">
                    <!-- Search Input -->
                    <div class="relative w-full sm:w-80">
                        ...
                        <input
                            ...
                            class="w-full pl-11 pr-4 py-3 rounded-full border border-slate-200 bg-white text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:placeholder-slate-500 transition"
                        >
                        <?php if($search): ?>
                            <button wire:click="$set('search', '')" class="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-400 hover:text-slate-600 dark:hover:text-slate-300">
                    ...
                    <!-- Framework Filter -->
                    <?php if($this->frameworks->count() > 1): ?>
                        <div class="relative w-full sm:w-48">
                            <select
                                wire:model.live="framework"
                                class="w-full appearance-none pl-4 pr-10 py-3 rounded-full border border-slate-200 bg-white text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:border-slate-700 dark:bg-slate-800 dark:text-white cursor-pointer transition"
                            >
                                <option value="">All Frameworks</option>
                                <?php $__currentLoopData = $this->frameworks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $fw): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($fw); ?>"><?php echo e($fw); ?></option>
                    ...
                    <?php if($search || $framework): ?>
                        <button
                            wire:click="clearFilters"
                            class="inline-flex items-center gap-2 px-4 py-3 rounded-full text-sm font-medium text-slate-600 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-white dark:hover:bg-slate-800 transition"
                        >
                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                            </svg>
                ...
                <?php if($search || $framework): ?>
                    <div class="mt-6 text-center text-sm text-slate-500 dark:text-slate-400">
                        Found <?php echo e($this->projects->count()); ?> <?php echo e($this->projects->count() === 1 ? 'project' : 'projects'); ?>

                        <?php if($search): ?> matching "<?php echo e($search); ?>"<?php endif; ?>
                        <?php if($framework): ?> in <?php echo e($framework); ?><?php endif; ?>
                    </div>
                <?php endif; ?>
        ...
        <?php if($this->projects->count() > 0): ?>
                <div class="mt-16 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                    <?php $__currentLoopData = $this->projects; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $project): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <a href="<?php echo e(route('project.public', $project->slug)); ?>" wire:navigate class="group relative flex flex-col rounded-3xl border border-slate-200 bg-gradient-to-b from-white to-slate-50 p-8 shadow-xl shadow-slate-200/50 transition hover:-translate-y-1 hover:shadow-2xl dark:border-slate-800 dark:from-slate-900 dark:to-slate-950 dark:shadow-none">
                            <div class="flex items-center justify-between">
                                <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-600/10 text-blue-700 dark:bg-blue-500/20 dark:text-blue-200">
                                    <?php if($project->framework === 'Laravel'): ?>
                            ...
                            <div class="mt-6">
                                ...
                                <p class="mt-2 text-sm font-medium uppercase tracking-wide text-blue-600 dark:text-blue-300">
                                    <?php echo e($project->framework ?? 'Framework'); ?>
                            ...
                            <dl class="mt-6 space-y-3 text-sm text-slate-600 dark:text-slate-400">
                                <?php if($primaryDomain): ?>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="truncate"><?php echo e($primaryDomain); ?></span>
                                    </div>
                                <?php endif; ?>
                                <?php if($project->php_version): ?>
                                    <div class="flex items-center gap-2">
                                        <svg class="h-4 w-4 text-purple-500 dark:text-purple-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"></path>
                                        </svg>
                                        <span>PHP <?php echo e($project->php_version); ?></span>
                                    </div>
                                <?php endif; ?>
                                <?php if($project->environment): ?>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span><?php echo e(ucfirst($project->environment)); ?> environment</span>
                                    </div>
                                <?php endif; ?>
                            </dl>
        ...
        <?php else: ?>
                <div class="mt-16 flex flex-col items-center justify-center rounded-3xl border border-dashed border-slate-300 p-16 text-center dark:border-slate-700">
                    ...
                    <?php if(auth()->guard()->check()): ?>
                        <a href="<?php echo e(route('projects.create')); ?>" class="mt-8 inline-flex items-center rounded-full bg-slate-900 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-slate-900/10 transition hover:-translate-y-0.5 hover:bg-slate-800 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-200">
        ...
        <section class="py-24">
            <div class="mx-auto w-full max-w-[1440px] overflow-hidden rounded-3xl bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 px-6 py-16 shadow-2xl md:px-12 lg:flex lg:items-center lg:justify-between lg:px-20">
                ...
                <div class="mt-10 lg:mt-0">
                    <?php if(auth()->guard()->check()): ?>
                        <a href="<?php echo e(route('projects.index')); ?>" class="inline-flex items-center justify-center rounded-full bg-white px-8 py-3 text-sm font-semibold text-slate-900 shadow-lg shadow-slate-900/20 transition hover:-translate-y-0.5 hover:shadow-2xl">
                            Manage Projects
                        </a>
                    <?php else: ?>
                        <a href="<?php echo e(route('login')); ?>" class="inline-flex items-center justify-center rounded-full bg-white px-8 py-3 text-sm font-semibold text-slate-900 shadow-lg shadow-slate-900/20 transition hover:-translate-y-0.5 hover:shadow-2xl">
                            Request Access
                        </a>
                    <?php endif; ?>
                </div>
            ...
            <div class="flex flex-col items-center justify-between gap-6 md:flex-row">
                ...
                <div class="flex flex-col items-center md:items-end gap-2">
                    <p class="text-xs text-slate-500 dark:text-slate-500">
                        &copy; <?php echo e(date('Y')); ?> NileStack. All rights reserved.
                    </p>
                </div>
            </div>
        </div>
...
<?php /**PATH /home/roshdy/Work/projects/DEVFLOW_PRO/resources/views/livewire/home/home-public.blade.php ENDPATH**/ ?>
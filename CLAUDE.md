...
|------------------------|------------------------------------------|
| Bug fixes (known file) | Direct: Read → Edit → Bash (test)        |
| UI tweaks              | Direct: Read → Edit → Bash (npm build)   |
| Test failures          | Direct: Bash (test) → Read → Edit        |
| New feature (small)    | Direct: Read → Write → Edit              |
| New feature (large)    | Task: Plan agent → implement agent       |
| Codebase exploration   | Task: Explore agent                      |
| Code review            | Task: code-reviewer agent                |
| Multi-file refactor    | Task: Plan → parallel implement agents   |

**Key Insight**: Direct tool calls are most efficient for focused tasks with known file locations. Task agents are better for exploration or multi-step implementations requiring planning.

#### Testing Best Practices

##### 1. Locale Handling (CRITICAL)
This project uses locale-aware middleware. **Always set locale in test setup:**
...
**Why**: The `SetLocale` middleware defaults to Arabic, causing assertion failures on English strings.

##### 2. Test Execution Strategy
```bash
# Run specific tests first for fast feedback
php artisan test --filter=ComponentName

# Run full suite before commit (use sequential if parallel fails)
php artisan test

# Parallel test failures are often infrastructure, not code issues
```

##### 3. Translation Key Verification
Before asserting translation strings exist:
```bash
# Use Grep to verify strings exist in actual lang files
grep -r "your_translation_key" lang/
```

**Common issue**: `navigation.*` vs `messages.*` key confusion.

#### Asset Building Requirements

After CSS/JS changes, **always run**:
```bash
npm run build
```
...
- Flatpickr/third-party component styling
- Any `resources/css` or `resources/js` modifications

#### Dark Mode Testing Checklist

When modifying any UI component, verify both themes:
- [ ] CSS color variables work in both modes
- [ ] Native HTML elements (select, input) styled correctly
- [ ] Third-party components (Flatpickr, etc.) themed properly
...
|-------------------------|-------------------------------------------|-------------------------------------------------------|-----------------------------------|
| Test failures (locale)  | SetLocale middleware defaults to Arabic   | Add `session(['locale' => 'en'])` to test beforeEach  | Always set locale in test setup   |
| Wrong translation key   | navigation.* vs messages.*                | Check actual file content before asserting            | Use Grep to verify strings exist  |
| RTL CSS issues          | Unnecessary RTL-specific overrides        | Trust Tailwind's ps-/pe- logical properties           | Remove redundant RTL CSS          |
| Asset changes not shown | Missing build step                        | Run `npm run build`                                   | Always build after CSS/JS changes |

#### Recommended Workflow for UI/Feature Tasks

1. **READ** affected files first (understand existing patterns)
2. **EDIT** with minimal changes (avoid over-engineering)
3. **BUILD** assets (`npm run build`)
4. **TEST** in browser (manual verification for visual changes)
5. **TEST** automated (`php artisan test --filter=X`)
6. **FIX** any failures
7. **UPDATE** documentation (CLAUDE.md if patterns changed)
...
- Run before committing: `./vendor/bin/phpstan analyse`
- Common fixes:

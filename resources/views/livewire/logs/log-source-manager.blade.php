            ...
            <div class="flex items-center justify-between">
                <div>
                    <nav class="flex mb-2" aria-label="Breadcrumb">
                        <ol class="inline-flex items-center space-x-1 md:space-x-3">
                            <li class="inline-flex items-center">
                                <a href="{{ route('servers.index') }}" class="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300">
                                    Servers
                                </a>
                            </li>
                            <li>
                                <div class="flex items-center">
                                    ...
                                    <span class="ml-1 text-gray-700 dark:text-gray-300">{{ $server->name }}</span>
                                </div>
                            </li>
                        </ol>
                    </nav>
                    ...
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                        Manage log collection sources for {{ $server->name }}
                    </p>
                </div>
                <button
                    ...
                    <li class="px-6 py-4 hover:bg-gray-50 dark:hover:bg-gray-750">
                        <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <h3 class="text-sm font-medium text-gray-900 dark:text-white">
                                            {{ $source->name }}
                                            @if($source->project)
                                                <span class="text-xs text-gray-500 dark:text-gray-400">({{ $source->project->name }})</span>
                                            @endif
                                        </h3>
                                        ...
                                        @if($source->last_synced_at)
                                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                                Last synced: {{ $source->last_synced_at->diffForHumans() }}
                                            </p>
                                        @endif
                                    </div>
                                </div>
                            ...
                            <div class="flex items-center space-x-2 ml-4">
                                ...
                                <button
                                    wire:click="removeSource({{ $source->id }})"
                ...
                <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
                    <div>
                        <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white">
                            {{ $editingSourceId ? 'Edit Log Source' : 'Add Log Source' }}
                        </h3>
                                ...
                                <div class="grid grid-cols-2 gap-2">
                                    @foreach($this->templates as $key => $template)
                                        <button
                                            ...
                                            class="px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 {{ $selectedTemplate === $key ? 'ring-2 ring-blue-500 bg-blue-50 dark:bg-blue-900/20' : '' }}">
                                            {{ $template['name'] }}
                                        </button>
                                    @endforeach
                                </div>
                        ...
                        <form wire:submit.prevent="{{ $editingSourceId ? 'updateSource' : 'addSource' }}" class="mt-6 space-y-4">
                            <!-- Name -->
                            <div>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                    {{ $type === 'docker' ? 'Container Name' : ($type === 'journald' ? 'Unit Name' : 'File Path') }}
                                </label>
                                <input
                                    type="text"
                                    wire:model="path"
                                    placeholder="{{ $type === 'docker' ? 'container_name' : ($type === 'journald' ? 'nginx.service' : '/var/log/nginx/error.log') }}"
                                ...
                                @error('path') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                            </div>
                            ...
                            <div>
                                ...
                                @error('project_id') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                            </div>

                            <!-- Test Connection -->
                            @if($testResult)
                                <div class="rounded-md p-4 {{ $testResult === 'success' ? 'bg-green-50 dark:bg-green-900/20' : 'bg-red-50 dark:bg-red-900/20' }}">
                                    <div class="flex">
                                        <div class="flex-shrink-0">
                                        ...
                                        <div class="ml-3">
                                            <p class="text-sm {{ $testResult === 'success' ? 'text-green-800 dark:text-green-300' : 'text-red-800 dark:text-red-300' }}">

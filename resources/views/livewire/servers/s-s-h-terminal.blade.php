<div class="space-y-6">
                ...
                <div class="bg-gray-900 rounded-lg overflow-hidden">
                    ...
                    <div class="px-4 py-2 bg-gray-800 flex items-center justify-between">
                        <div class="flex items-center space-x-3">
                            <span class="text-green-400 font-mono text-sm">$</span>
                            <span class="text-white font-mono text-sm">{{ $item['command'] }}</span>
                            @if($item['success'])
                        ...
                        <div class="flex items-center space-x-2">
                            <span class="text-xs text-gray-500">{{ \Carbon\Carbon::parse($item['timestamp'])->diffForHumans() }}</span>
                            <button
                                wire:click="rerunCommand({{ $index }})"
                    ...
                    <div class="px-4 py-3 bg-gray-900">
                        <pre class="text-sm text-gray-300 font-mono whitespace-pre-wrap overflow-x-auto max-h-96 overflow-y-auto">{{ $item['output'] }}</pre>
                    </div>
                </div>
            @endforeach
        </div>
    ...
    <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
        ...
        <ul class="text-xs text-blue-800 dark:text-blue-300 space-y-1">
            <li>• Commands execute with user: <code class="px-1 py-0.5 bg-blue-100 dark:bg-blue-900/50 rounded">{{ $server->username }}</code></li>

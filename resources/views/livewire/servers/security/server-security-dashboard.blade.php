        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                <div class="flex items-center gap-4">
                    <a href="{{ route('servers.show', $server) }}" class="p-2 bg-white/10 rounded-lg hover:bg-white/20 transition-colors">
                    ...
                    <div>
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-white/80">{{ $server->name }} ({{ $server->ip_address }})</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="flex flex-wrap gap-3">
                    ...
                    <a href="{{ route('servers.security.hardening', $server) }}"
                        class="px-4 py-2 bg-amber-500 hover:bg-amber-400 text-white font-medium rounded-lg transition-colors flex items-center gap-2">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                ...
                <div class="flex flex-col items-center">
                    ...
                    @if($server->last_security_scan_at)
                        <p class="mt-2 text-xs text-gray-500">
                            Last scan: {{ $server->last_security_scan_at->diffForHumans() }}
                        </p>
                    @endif
                </div>
            </div>
            ...
            <div class="lg:col-span-2 grid grid-cols-2 sm:grid-cols-4 gap-4">
                <!-- UFW Status -->
                <a href="{{ route('servers.security.firewall', $server) }}"
                   class="bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700/50 p-4 hover:border-orange-500/50 transition-colors">
                    <div class="flex items-center justify-between mb-2">
                        <svg class="w-8 h-8 {{ $server->ufw_enabled ? 'text-green-400' : 'text-red-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 18.657A8 8 0 016.343 7.343S7 9 9 10c0-2 .5-5 2.986-7C14 5 16.09 5.777 17.656 7.343A7.975 7.975 0 0120 13a7.975 7.975 0 01-2.343 5.657z"/>
                        </svg>
                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium {{ $server->ufw_enabled ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400' }}">
                            {{ $server->ufw_enabled ? 'Active' : 'Inactive' }}
                        </span>
                    </div>
                    <h4 class="text-white font-medium">Firewall</h4>
                    <p class="text-gray-400 text-sm">UFW {{ $server->ufw_installed ? 'installed' : 'not installed' }}</p>
                </a>

                <!-- Fail2ban Status -->
                <a href="{{ route('servers.security.fail2ban', $server) }}"
                   class="bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700/50 p-4 hover:border-orange-500/50 transition-colors">
                    <div class="flex items-center justify-between mb-2">
                        <svg class="w-8 h-8 {{ $server->fail2ban_enabled ? 'text-green-400' : 'text-red-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"/>
                        </svg>
                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium {{ $server->fail2ban_enabled ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400' }}">
                            {{ $server->fail2ban_enabled ? 'Active' : 'Inactive' }}
                        </span>
                    </div>
                    <h4 class="text-white font-medium">Fail2ban</h4>
                    <p class="text-gray-400 text-sm">{{ $server->fail2ban_installed ? 'Installed' : 'Not installed' }}</p>
                </a>

                <!-- SSH Status -->
                <a href="{{ route('servers.security.ssh', $server) }}"
                   class="bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700/50 p-4 hover:border-orange-500/50 transition-colors">
                    <div class="flex items-center justify-between mb-2">
                    ...
                    <p class="text-gray-400 text-sm">Port {{ $server->port }}</p>
                </a>
                ...
                <div class="bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700/50 p-4">
                    ...
                    <p class="text-gray-400 text-sm">{{ $securityOverview['open_ports']['count'] ?? '--' }} ports</p>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            ...
            <a href="{{ route('servers.security.scan', $server) }}"
                    ...
                    @foreach($this->recentEvents as $event)
                        <div class="flex items-start gap-4 p-3 rounded-lg bg-gray-900/50">
                            <div class="p-2 rounded-lg
                                {{ $event->event_type_color === 'green' ? 'bg-green-500/20' : '' }}
                                {{ $event->event_type_color === 'red' ? 'bg-red-500/20' : '' }}
                                {{ $event->event_type_color === 'orange' ? 'bg-orange-500/20' : '' }}
                                {{ $event->event_type_color === 'yellow' ? 'bg-yellow-500/20' : '' }}
                                {{ $event->event_type_color === 'blue' ? 'bg-blue-500/20' : '' }}
                                {{ $event->event_type_color === 'purple' ? 'bg-purple-500/20' : '' }}
                                {{ $event->event_type_color === 'gray' ? 'bg-gray-500/20' : '' }}">
                                <svg class="w-5 h-5
                                    {{ $event->event_type_color === 'green' ? 'text-green-400' : '' }}
                                    {{ $event->event_type_color === 'red' ? 'text-red-400' : '' }}
                                    {{ $event->event_type_color === 'orange' ? 'text-orange-400' : '' }}
                                    {{ $event->event_type_color === 'yellow' ? 'text-yellow-400' : '' }}
                                    {{ $event->event_type_color === 'blue' ? 'text-blue-400' : '' }}
                                    {{ $event->event_type_color === 'purple' ? 'text-purple-400' : '' }}
                                    {{ $event->event_type_color === 'gray' ? 'text-gray-400' : '' }}"
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center justify-between gap-2">
                                    <h4 class="text-white font-medium truncate">{{ $event->getEventTypeLabel() }}</h4>
                                    <span class="text-xs text-gray-500 whitespace-nowrap">{{ $event->created_at->diffForHumans() }}</span>
                                </div>
                                @if($event->details)
                                    <p class="text-gray-400 text-sm truncate">{{ $event->details }}</p>
                                @endif
                                @if($event->source_ip)
                                    <p class="text-gray-500 text-xs">IP: {{ $event->source_ip }}</p>
                                @endif
                            </div>
                        </div>
                    @endforeach

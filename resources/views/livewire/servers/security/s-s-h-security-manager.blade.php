                    ...
                    <div>
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-white/80">{{ $server->name }} - SSH Configuration</p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center justify-between p-4 bg-gray-900/50 rounded-lg">
                        <div>
                            ...
                            <p class="text-gray-400 text-sm">Current port: {{ $port }}</p>
                        </div>
                    ...
                    <div class="flex items-center justify-between p-4 bg-gray-900/50 rounded-lg">
                        ...
                        <button wire:click="toggleRootLogin"
                            class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors {{ $rootLoginEnabled ? 'bg-green-600' : 'bg-gray-600' }}">
                            <span class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform {{ $rootLoginEnabled ? 'translate-x-6' : 'translate-x-1' }}"></span>
                        </button>
                    </div>
                    ...
                    <div class="flex items-center justify-between p-4 bg-gray-900/50 rounded-lg">
                        ...
                        <button wire:click="togglePasswordAuth"
                            class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors {{ $passwordAuthEnabled ? 'bg-green-600' : 'bg-gray-600' }}">
                            <span class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform {{ $passwordAuthEnabled ? 'translate-x-6' : 'translate-x-1' }}"></span>
                        </button>
                    </div>
                    ...
                    <div class="flex items-center justify-between p-4 bg-gray-900/50 rounded-lg">
                        ...
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $pubkeyAuthEnabled ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400' }}">
                            {{ $pubkeyAuthEnabled ? 'Enabled' : 'Disabled' }}
                        </span>
                    </div>
                    ...
                    <div class="flex items-center justify-between p-4 bg-gray-900/50 rounded-lg">
                        ...
                        <span class="text-white font-medium">{{ $maxAuthTries }}</span>
                    </div>
                </div>
            ...
            <div class="bg-gray-800/50 backdrop-blur-sm rounded-2xl border border-gray-700/50 p-6">
                ...
                <div class="space-y-4">
                    <!-- Port Check -->
                    <div class="flex items-start gap-3 p-4 rounded-lg {{ $port !== 22 ? 'bg-green-900/20 border border-green-700/50' : 'bg-yellow-900/20 border border-yellow-700/50' }}">
                        <svg class="w-6 h-6 {{ $port !== 22 ? 'text-green-400' : 'text-yellow-400' }} flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            @if($port !== 22)
                        ...
                        <div>
                            <h4 class="text-white font-medium">{{ $port !== 22 ? 'Non-standard port in use' : 'Using default SSH port' }}</h4>
                            <p class="text-gray-400 text-sm">{{ $port !== 22 ? 'Good! Using a non-standard port reduces automated attacks.' : 'Consider changing from port 22 to reduce automated attacks.' }}</p>
                        </div>
                    </div>

                    <!-- Root Login Check -->
                    <div class="flex items-start gap-3 p-4 rounded-lg {{ !$rootLoginEnabled ? 'bg-green-900/20 border border-green-700/50' : 'bg-red-900/20 border border-red-700/50' }}">
                        <svg class="w-6 h-6 {{ !$rootLoginEnabled ? 'text-green-400' : 'text-red-400' }} flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            @if(!$rootLoginEnabled)
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                            @else
                        ...
                        <div>
                            <h4 class="text-white font-medium">{{ !$rootLoginEnabled ? 'Root login disabled' : 'Root login enabled' }}</h4>
                            <p class="text-gray-400 text-sm">{{ !$rootLoginEnabled ? 'Good! Root login is disabled for security.' : 'Disable root login and use a regular user with sudo.' }}</p>
                        </div>
                    </div>

                    <!-- Password Auth Check -->
                    <div class="flex items-start gap-3 p-4 rounded-lg {{ !$passwordAuthEnabled ? 'bg-green-900/20 border border-green-700/50' : 'bg-yellow-900/20 border border-yellow-700/50' }}">
                        <svg class="w-6 h-6 {{ !$passwordAuthEnabled ? 'text-green-400' : 'text-yellow-400' }} flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div>
                            <h4 class="text-white font-medium">{{ !$passwordAuthEnabled ? 'Key-only authentication' : 'Password authentication enabled' }}</h4>
                            <p class="text-gray-400 text-sm">{{ !$passwordAuthEnabled ? 'Good! Only SSH keys are allowed for login.' : 'Consider using SSH keys only for better security.' }}</p>
                        </div>
                    </div>

                    <!-- Max Auth Tries Check -->
                    <div class="flex items-start gap-3 p-4 rounded-lg {{ $maxAuthTries <= 3 ? 'bg-green-900/20 border border-green-700/50' : 'bg-yellow-900/20 border border-yellow-700/50' }}">
                        <svg class="w-6 h-6 {{ $maxAuthTries <= 3 ? 'text-green-400' : 'text-yellow-400' }} flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            @if($maxAuthTries <= 3)
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                            @else
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
                        ...
                        <div>
                            <h4 class="text-white font-medium">{{ $maxAuthTries <= 3 ? 'Low auth tries limit' : 'High auth tries limit' }}</h4>
                            <p class="text-gray-400 text-sm">{{ $maxAuthTries <= 3 ? 'Good! Limited attempts reduce brute force risk.' : 'Consider lowering MaxAuthTries to 3 or less.' }}</p>
                        </div>
                    </div>
                </div>
            </div>

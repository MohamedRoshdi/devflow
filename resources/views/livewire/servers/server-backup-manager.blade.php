                    ...
                    <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                        <div class="flex items-center gap-4">
                            <div class="w-12 h-12 rounded-lg bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center text-white font-bold">
                                {{ strtoupper(substr($schedule->type, 0, 1)) }}
                            </div>
                            <div>
                                <p class="font-semibold text-gray-900 dark:text-white">{{ ucfirst($schedule->type) }} Backup</p>
                                <p class="text-sm text-gray-600 dark:text-gray-400">{{ $schedule->frequency_description }}</p>
                                <p class="text-xs text-gray-500 dark:text-gray-500 mt-1">
                                    Retention: {{ $schedule->retention_days }} days | Storage: {{ strtoupper($schedule->storage_driver) }}
                                    @if($schedule->next_run)
                                        | Next run: {{ $schedule->next_run->diffForHumans() }}
                                    @endif
                                </p>
                            </div>
                        </div>
                        <div class="flex items-center gap-2">
                            <span class="px-3 py-1 rounded-full text-xs font-semibold
                                @if($schedule->is_active) bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold
                                        @if($backup->type === 'full') bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400
                                        @elseif($backup->type === 'incremental') bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400
                                        @else bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400
                                        @endif">
                                        {{ ucfirst($backup->type) }}
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold
                                        @if($backup->status === 'completed') bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400
                                        @elseif($backup->status === 'running') bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400
                                        @elseif($backup->status === 'failed') bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center text-xs font-medium text-gray-700 dark:text-gray-300">
                                        {{ strtoupper($backup->storage_driver) }}
                                    </span>
                                </td>
                                    ...
                                    <div class="flex items-center justify-end gap-2">
                                        @if($backup->isCompleted())
                                            ...
                                            @if($backup->storage_driver === 'local')
                                                <button wire:click="uploadToS3({{ $backup->id }})"
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        ...
                        @error('backupType') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('storageDriver') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        ...
                        @error('scheduleType') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('scheduleFrequency') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="time" wire:model="scheduleTime"
                               class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500">
                        @error('scheduleTime') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                    </div>
                    ...
                    @if($scheduleFrequency === 'weekly')
                        <div>
                            ...
                            @error('scheduleDayOfWeek') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                        </div>
                    @endif
                    ...
                    @if($scheduleFrequency === 'monthly')
                        <div>
                            ...
                            <input type="number" wire:model="scheduleDayOfMonth" min="1" max="31"
                                   class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500"
                                   placeholder="1-31">
                            @error('scheduleDayOfMonth') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                        </div>
                    @endif
                    ...
                    <div>
                        ...
                        <input type="number" wire:model="retentionDays" min="1" max="365"
                               class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500"
                               placeholder="30">
                        @error('retentionDays') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('scheduleStorageDriver') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                    </div>
                </div>

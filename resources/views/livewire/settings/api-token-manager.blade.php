<div>
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                    @if($token->last_used_at)
                                        <span class="text-green-600 dark:text-green-400">{{ $token->last_used_at->diffForHumans() }}</span>
                                    @else
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    @if($token->expires_at)
                                        ...
                                        @else
                                            <span class="text-yellow-600 dark:text-yellow-400">{{ $token->expires_at->format('M d, Y') }}</span>
                                        @endif
                                    @else
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        @error('newTokenName') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            @foreach($availableAbilities as $ability => $description)
                                <label class="flex items-start p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer transition-colors">
                                    <input type="checkbox" wire:model="newTokenAbilities" value="{{ $ability }}"
                                           class="mt-1 rounded border-gray-300 dark:border-gray-600 text-blue-600 focus:ring-blue-500 dark:bg-gray-700">
                                    <div class="ml-3">
                                        <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $ability }}</div>
                                        <div class="text-xs text-gray-500 dark:text-gray-400">{{ $description }}</div>
                                    </div>
                                </label>
                            @endforeach
                        </div>
                        @error('newTokenAbilities') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                    </div>
    ...
    <!-- Token Display Modal -->
    @if($showTokenModal && $createdTokenPlain)
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-2xl w-full mx-4">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="text" value="{{ $createdTokenPlain }}" readonly
                            ...
                            <button onclick="navigator.clipboard.writeText('{{ $createdTokenPlain }}')"

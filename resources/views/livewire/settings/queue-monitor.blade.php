<div>
    ...
    <div class="relative mb-8 rounded-2xl bg-gradient-to-br from-purple-500 via-pink-500 to-red-500 dark:from-purple-600 dark:via-pink-600 dark:to-red-600 p-8 shadow-xl overflow-hidden">
        ...
        <div class="relative z-10 flex justify-between items-center">
            ...
            <button wire:click="refreshStats"
                ...
                <svg class="w-5 h-5 inline mr-2 {{ $isLoading ? 'animate-spin' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900 dark:text-white mt-2">
                        {{ number_format($queueStats['pending_jobs'] ?? 0) }}
                    </p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border-l-4 border-blue-500">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900 dark:text-white mt-2">
                        {{ number_format($queueStats['processing_jobs'] ?? 0) }}
                    </p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border-l-4 border-red-500">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900 dark:text-white mt-2">
                        {{ number_format($queueStats['failed_jobs'] ?? 0) }}
                    </p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border-l-4 border-green-500">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900 dark:text-white mt-2">
                        {{ number_format($queueStats['jobs_per_hour'] ?? 0) }}
                    </p>
                </div>
        ...
        <!-- Worker Status -->
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border-l-4 {{ ($queueStats['worker_status']['is_running'] ?? false) ? 'border-green-500' : 'border-gray-500' }}">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Worker Status</p>
                    <p class="text-lg font-bold {{ ($queueStats['worker_status']['is_running'] ?? false) ? 'text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400' }} mt-2">
                        {{ $queueStats['worker_status']['status_text'] ?? 'Unknown' }}
                    </p>
                </div>
                <div class="p-3 {{ ($queueStats['worker_status']['is_running'] ?? false) ? 'bg-green-100 dark:bg-green-900/30' : 'bg-gray-100 dark:bg-gray-700' }} rounded-lg">
                    <div class="relative">
                        <svg class="w-8 h-8 {{ ($queueStats['worker_status']['is_running'] ?? false) ? 'text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z"></path>
                        </svg>
                        @if($queueStats['worker_status']['is_running'] ?? false)
                        ...
                        @foreach($failedJobs as $job)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="text-sm font-mono text-gray-900 dark:text-white">{{ $job['id'] }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900 dark:text-white font-medium max-w-xs truncate" title="{{ $job['job_class'] }}">
                                        {{ $job['job_class'] }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900 dark:text-white">{{ $job['failed_at']->format('M d, Y H:i') }}</div>
                                    <div class="text-xs text-gray-500 dark:text-gray-400">{{ $job['failed_at_human'] }}</div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="max-w-md">
                                        <p class="text-sm text-gray-700 dark:text-gray-300 truncate" title="{{ $job['short_exception'] }}">
                                            {{ Str::limit($job['short_exception'], 100) }}
                                        </p>
    ...
    <!-- Job Details Modal -->
    @if($showJobDetails && !empty($selectedJob))
                    ...
                    <div class="flex-1">
                        ...
                        <div class="flex flex-wrap gap-3 text-sm">
                            <div class="flex items-center">
                                ...
                                <span class="font-mono text-gray-900 dark:text-white">{{ $selectedJob['id'] }}</span>
                            </div>
                            <div class="flex items-center">
                            ...
                            <div class="flex items-center">
                                ...
                                <span class="text-gray-900 dark:text-white">{{ $selectedJob['failed_at']->format('M d, Y H:i:s') }}</span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex-1 overflow-y-auto p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <div class="bg-gray-50 dark:bg-gray-900 rounded-lg p-4">
                            <code class="text-sm text-gray-900 dark:text-white font-mono break-all">{{ $selectedJob['job_class'] }}</code>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="bg-gray-50 dark:bg-gray-900 rounded-lg p-4 max-h-96 overflow-y-auto">
                            <pre class="text-xs text-red-600 dark:text-red-400 font-mono whitespace-pre-wrap break-words">{{ $selectedJob['exception'] }}</pre>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="bg-gray-50 dark:bg-gray-900 rounded-lg p-4">
                            <code class="text-sm text-gray-900 dark:text-white font-mono">{{ $selectedJob['uuid'] }}</code>
                        </div>
                    </div>
                </div>

...
<div wire:key="project-show-{{ $project->id }}"
    ...
    <div class="relative">
                        ...
                        <div class="flex flex-wrap items-center gap-3">
                            ...
                            <div class="flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-sm rounded-full border border-white/10">
                                <button wire:click="toggleAutoRefresh"
                                        class="relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none {{ $autoRefreshEnabled ? 'bg-emerald-500' : 'bg-white/30' }}"
                                        role="switch"
                                        aria-checked="{{ $autoRefreshEnabled ? 'true' : 'false' }}"
                                        title="{{ $autoRefreshEnabled ? 'Disable auto-refresh' : 'Enable auto-refresh' }}">
                                    <span class="pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out {{ $autoRefreshEnabled ? 'translate-x-4' : 'translate-x-0' }}"></span>
                                </button>
                                ...
                                @if($autoRefreshEnabled)
                                    <select wire:change="setAutoRefreshInterval($event.target.value)"
                                        ...
                                        <option value="10" {{ $autoRefreshInterval == 10 ? 'selected' : '' }} class="bg-slate-800 text-white">10s</option>
                                        <option value="30" {{ $autoRefreshInterval == 30 ? 'selected' : '' }} class="bg-slate-800 text-white">30s</option>
                                        <option value="60" {{ $autoRefreshInterval == 60 ? 'selected' : '' }} class="bg-slate-800 text-white">1m</option>
                                        <option value="120" {{ $autoRefreshInterval == 120 ? 'selected' : '' }} class="bg-slate-800 text-white">2m</option>
                                        <option value="300" {{ $autoRefreshInterval == 300 ? 'selected' : '' }} class="bg-slate-800 text-white">5m</option>
                                    </select>
                                @endif
                            </div>
                    ...
                    @if($lastGitRefreshAt)
                        <div class="mt-4 pt-4 border-t border-white/10 flex flex-wrap items-center justify-between gap-2">
                            <div class="flex items-center gap-2 text-xs text-white/60">
                                ...
                                <span>Last updated: <span class="text-white/80 font-medium">{{ \Carbon\Carbon::parse($lastGitRefreshAt)->diffForHumans() }}</span></span>
                            </div>
                            @if($autoRefreshEnabled)
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-emerald-500/20 text-emerald-300 rounded-full text-xs font-medium">
                                    ...
                                    Auto-refresh: {{ $autoRefreshInterval < 60 ? $autoRefreshInterval . 's' : ($autoRefreshInterval / 60) . 'm' }}
                                </span>
                            @else
        ...
        <div class="space-y-8">
            ...
            @livewire('deployments.scheduled-deployments', ['project' => $project], key('scheduled-' . $project->id))

            <!-- Rollback Section -->
            @livewire('deployments.deployment-rollback', ['project' => $project], key('rollback-' . $project->id))

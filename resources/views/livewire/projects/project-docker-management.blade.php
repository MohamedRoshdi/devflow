    ...
    <div class="border-b border-gray-200">
        <nav class="flex px-6 -mb-px space-x-8">
            <button wire:click="switchTab('overview')" 
                    ...
                    @if($activeTab === 'overview') border-blue-500 text-blue-600 @else border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 @endif">
                Overview
            </button>
            <button wire:click="switchTab('images')" 
                    class="py-4 px-1 border-b-2 font-medium text-sm transition
                    @if($activeTab === 'images') border-blue-500 text-blue-600 @else border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 @endif">
                Images ({{ count($images) }})
            </button>
            <button wire:click="switchTab('logs')" 
                    class="py-4 px-1 border-b-2 font-medium text-sm transition
                    @if($activeTab === 'logs') border-blue-500 text-blue-600 @else border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 @endif">
                Container Logs
            </button>
        </nav>
    </div>
                ...
                <div class="space-y-6">
                            ...
                            <div class="space-y-3">
                                ...
                                @if(isset($containerInfo['Ports']))
                                    <div class="flex justify-between items-center">
                                        ...
                                        <span class="text-gray-900">{{ $containerInfo['Ports'] }}</span>
                                    </div>
                                @endif
                                ...
                                <div class="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
                                    @if(isset($containerStats['CPUPerc']))
                                        <div class="bg-white rounded-lg p-4 border border-blue-200">
                                            ...
                                            <p class="text-lg font-bold text-gray-900">{{ $containerStats['CPUPerc'] }}</p>
                                        </div>
                                    @endif
                                    @if(isset($containerStats['MemPerc']))
                                        <div class="bg-white rounded-lg p-4 border border-blue-200">
                                            <p class="text-xs text-gray-600 mb-1">Memory Usage</p>
                                            <p class="text-lg font-bold text-gray-900">{{ $containerStats['MemPerc'] }}</p>
                                        </div>
                                    @endif
                                    @if(isset($containerStats['NetIO']))
                                        <div class="bg-white rounded-lg p-4 border border-blue-200">
                                            <p class="text-xs text-gray-600 mb-1">Network I/O</p>
                                            <p class="text-sm font-bold text-gray-900">{{ $containerStats['NetIO'] }}</p>
                                        </div>
                                    @endif
                                    @if(isset($containerStats['BlockIO']))
                                        <div class="bg-white rounded-lg p-4 border border-blue-200">
                                            <p class="text-xs text-gray-600 mb-1">Disk I/O</p>
                                            <p class="text-sm font-bold text-gray-900">{{ $containerStats['BlockIO'] }}</p>
                                        </div>
                                    @endif
                                </div>
                            @endif
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div class="bg-white border border-gray-200 rounded-lg p-4">
                            ...
                            <p class="text-lg font-bold text-gray-900">{{ $project->server->name }}</p>
                            <p class="text-xs text-gray-500 mt-1">{{ $project->server->ip_address }}</p>
                        </div>
                    </div>
                </div>
            @endif
            ...
            @if($activeTab === 'images')
                <div>
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="text-lg font-semibold text-gray-900">Docker Images for {{ $project->name }}</h3>
                        <button wire:click="buildImage" 
                                class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition">
                            🔨 Build New Image
                        </button>
                                    ...
                                    @foreach($images as $image)
                                        <tr class="hover:bg-gray-50">
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <code class="text-sm text-gray-900">{{ $image['Repository'] ?? 'N/A' }}</code>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <span class="px-2 py-1 text-xs bg-blue-100 text-blue-800 rounded">{{ $image['Tag'] ?? 'latest' }}</span>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <code class="text-xs text-gray-600">{{ substr($image['ID'] ?? '', 0, 12) }}</code>
                                            </td>

    ...
    <div class="relative mb-8">
        <div class="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-3xl overflow-hidden border border-slate-700/50 shadow-2xl">
            ...
            <div class="relative p-6 lg:p-8">
                ...
                @if(count($deploymentSteps) > 0 && ($isDeploying || $deploymentStatus === 'success' || $deploymentStatus === 'failed'))
                    <div class="mt-6 p-6 rounded-2xl bg-slate-800/50 border border-slate-700/30 backdrop-blur-sm">
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-slate-400">
                                        @if($isDeploying)
                                            Step {{ $currentStep + 1 }} of {{ count($deploymentSteps) }}
                                        @else
                                            {{ $deploymentStatus === 'success' ? 'All steps completed successfully' : 'Check logs for details' }}
                                        @endif
                                    </p>
                                </div>
                            </div>
                            ...
                            {{-- Close Button --}}
                            @if(!$isDeploying)
                                <button wire:click="closeDeployment" class="p-2 rounded-lg text-slate-400 hover:text-white hover:bg-slate-700/50 transition-all">

                ...
                <div class="flex items-center gap-3">
                    @if($lastCheckedAt)
                        <span class="text-sm text-emerald-100">
                            Last checked: {{ \Carbon\Carbon::parse($lastCheckedAt)->diffForHumans() }}
                        </span>
                    @endif
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div class="w-12 h-12 rounded-full bg-white/20 flex items-center justify-center">
                                    <span class="text-xl font-bold text-white">{{ $project['health_score'] }}</span>
                                </div>
                                <div>
                                    <h3 class="font-bold text-white">{{ $project['name'] }}</h3>
                                    <p class="text-sm text-white/80">{{ $project['server_name'] }}</p>
                                </div>
                            </div>
                    ...
                    <div class="p-4 space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div class="p-3 rounded-lg bg-gray-50 dark:bg-gray-700/50">
                                ...
                                <div class="mt-1 font-semibold {{ $project['status'] === 'running' ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-600 dark:text-gray-400' }}">
                                    {{ ucfirst($project['status']) }}
                                </div>
                            </div>
                            <div class="p-3 rounded-lg bg-gray-50 dark:bg-gray-700/50">
                                <div class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">Uptime</div>
                                <div class="mt-1 font-semibold {{ $project['uptime_status'] === 'healthy' ? 'text-emerald-600 dark:text-emerald-400' : ($project['uptime_status'] === 'unhealthy' ? 'text-red-600 dark:text-red-400' : 'text-gray-500 dark:text-gray-400') }}">
                                    {{ ucfirst($project['uptime_status']) }}
                                </div>
                            </div>
                            <div class="p-3 rounded-lg bg-gray-50 dark:bg-gray-700/50">
                                ...
                                <div class="mt-1 font-semibold text-gray-900 dark:text-white">
                                    {{ $project['response_time'] ? $project['response_time'] . 'ms' : 'N/A' }}
                                </div>
                            </div>
                            <div class="p-3 rounded-lg bg-gray-50 dark:bg-gray-700/50">
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-4">
                                    <div class="w-10 h-10 rounded-full flex items-center justify-center
                                        {{ $server['health_score'] >= 80 ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400' : ($server['health_score'] >= 50 ? 'bg-amber-100 dark:bg-amber-900/40 text-amber-600 dark:text-amber-400' : 'bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400') }}">
                                        <span class="font-bold">{{ $server['health_score'] }}</span>
                                    </div>
                                    <div>
                                        <h3 class="font-semibold text-gray-900 dark:text-white">{{ $server['name'] }}</h3>
                                        <p class="text-sm text-gray-500 dark:text-gray-400">{{ $server['ip_address'] }} - {{ $server['projects_count'] }} projects</p>
                                    </div>
                                </div>
                                <div class="flex items-center gap-6 text-sm">
                                    @if($server['cpu_usage'] !== null)
                                        <div class="text-center">
                                            <div class="font-semibold {{ $server['cpu_usage'] > 80 ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-white' }}">{{ $server['cpu_usage'] }}%</div>
                                            <div class="text-xs text-gray-500 dark:text-gray-400">CPU</div>
                                        </div>
                                    @endif
                                    @if($server['ram_usage'] !== null)
                                        <div class="text-center">
                                            <div class="font-semibold {{ $server['ram_usage'] > 80 ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-white' }}">{{ $server['ram_usage'] }}%</div>
                                            <div class="text-xs text-gray-500 dark:text-gray-400">RAM</div>
                                        </div>
                                    @endif
                                    @if($server['disk_usage'] !== null)
                                        <div class="text-center">
                                            <div class="font-semibold {{ $server['disk_usage'] > 80 ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-white' }}">{{ $server['disk_usage'] }}%</div>

<div class="space-y-6">
    ...
    <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-gray-100 via-gray-50 to-white dark:from-slate-900/95 dark:via-slate-800/95 dark:to-slate-900/95 backdrop-blur-sm border border-gray-200 dark:border-slate-700/50 shadow-sm dark:shadow-none">
        {{-- Grid Pattern Overlay --}}
        <div class="absolute inset-0 bg-[linear-gradient(to_right,#6366f110_1px,transparent_1px),linear-gradient(to_bottom,#6366f110_1px,transparent_1px)] dark:bg-[linear-gradient(to_right,#4f46e520_1px,transparent_1px),linear-gradient(to_bottom,#4f46e520_1px,transparent_1px)] bg-[size:4rem_4rem]"></div>
        ...
        <div class="relative px-6 py-8 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="flex items-center gap-2 px-4 py-2 bg-purple-100 dark:bg-purple-500/10 backdrop-blur-sm border border-purple-200 dark:border-purple-500/20 rounded-lg">
                ...
                @endif
                <a href="{{ route('docs.show') }}" class="group relative overflow-hidden inline-flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-slate-800/50 backdrop-blur-sm border border-gray-200 dark:border-slate-700/50 hover:border-gray-300 dark:hover:border-slate-600 text-gray-700 dark:text-slate-300 font-medium rounded-lg transition-all">
    ...
    <div class="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 rounded-2xl p-[2px]">
        <div class="bg-white dark:bg-slate-900/95 backdrop-blur-sm rounded-2xl p-6">
            <div class="flex items-center justify-between mb-6">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500 dark:text-slate-400">Get your first deployment running in minutes</p>
                    </div>
                </div>
                <button wire:click="dismissGettingStarted" class="p-1 text-gray-400 dark:text-slate-400 hover:text-gray-600 dark:hover:text-slate-300 rounded">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {{-- Step 1 --}}
                <a href="{{ route('servers.create') }}" class="group relative flex items-center gap-4 p-4 rounded-xl border-2 transition-all {{ $onboardingSteps['add_server'] ? 'border-green-500 bg-green-50 dark:bg-green-500/10' : 'border-gray-200 dark:border-slate-700 hover:border-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-500/10' }}">
                    <div class="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center {{ $onboardingSteps['add_server'] ? 'bg-green-500 shadow-lg shadow-green-500/50' : 'bg-gray-200 dark:bg-slate-700 group-hover:bg-indigo-500 group-hover:shadow-lg group-hover:shadow-indigo-500/50' }} transition-all">
                        @if($onboardingSteps['add_server'])
                            <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                        @else
                            <span class="text-sm font-bold text-gray-600 dark:text-slate-300 group-hover:text-white">1</span>
                        @endif
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="font-medium text-gray-900 dark:text-white">Add Server</p>
                        <p class="text-xs text-gray-500 dark:text-slate-400 truncate">Connect via SSH</p>
                    </div>
                    @if(!$onboardingSteps['add_server'])
                    <svg class="w-5 h-5 text-gray-400 dark:text-slate-400 group-hover:text-indigo-500 dark:group-hover:text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                    @endif
                </a>

                {{-- Step 2 --}}
                <a href="{{ route('projects.create') }}" class="group relative flex items-center gap-4 p-4 rounded-xl border-2 transition-all {{ $onboardingSteps['create_project'] ? 'border-green-500 bg-green-50 dark:bg-green-500/10' : ($onboardingSteps['add_server'] ? 'border-gray-200 dark:border-slate-700 hover:border-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-500/10' : 'border-gray-100 dark:border-slate-800 opacity-50 pointer-events-none') }}">
                    <div class="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center {{ $onboardingSteps['create_project'] ? 'bg-green-500 shadow-lg shadow-green-500/50' : ($onboardingSteps['add_server'] ? 'bg-gray-200 dark:bg-slate-700 group-hover:bg-indigo-500 group-hover:shadow-lg group-hover:shadow-indigo-500/50' : 'bg-gray-100 dark:bg-slate-800') }} transition-all">
                        @if($onboardingSteps['create_project'])
                            <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                        @else
                            <span class="text-sm font-bold {{ $onboardingSteps['add_server'] ? 'text-gray-600 dark:text-slate-300 group-hover:text-white' : 'text-gray-400 dark:text-slate-600' }}">2</span>
                        @endif
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="font-medium text-gray-900 dark:text-white">Create Project</p>
                        <p class="text-xs text-gray-500 dark:text-slate-400 truncate">Link Git repository</p>
                    </div>
                </a>

                {{-- Step 3 --}}
                <div class="group relative flex items-center gap-4 p-4 rounded-xl border-2 {{ $onboardingSteps['first_deployment'] ? 'border-green-500 bg-green-50 dark:bg-green-500/10' : ($onboardingSteps['create_project'] ? 'border-gray-200 dark:border-slate-700' : 'border-gray-100 dark:border-slate-800 opacity-50') }}">
                    <div class="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center {{ $onboardingSteps['first_deployment'] ? 'bg-green-500 shadow-lg shadow-green-500/50' : ($onboardingSteps['create_project'] ? 'bg-gray-200 dark:bg-slate-700' : 'bg-gray-100 dark:bg-slate-800') }}">
                        @if($onboardingSteps['first_deployment'])
                            <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                        @else
                            <span class="text-sm font-bold text-gray-600 dark:text-slate-300">3</span>
                        @endif
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="font-medium text-gray-900 dark:text-white">Deploy</p>
                        <p class="text-xs text-gray-500 dark:text-slate-400 truncate">First deployment</p>
                    </div>
                </div>

                {{-- Step 4 --}}
                <a href="{{ route('projects.index') }}" class="group relative flex items-center gap-4 p-4 rounded-xl border-2 transition-all {{ $onboardingSteps['setup_domain'] ? 'border-green-500 bg-green-50 dark:bg-green-500/10' : ($onboardingSteps['first_deployment'] ? 'border-gray-200 dark:border-slate-700 hover:border-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-500/10' : 'border-gray-100 dark:border-slate-800 opacity-50 pointer-events-none') }}">
                    <div class="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center {{ $onboardingSteps['setup_domain'] ? 'bg-green-500 shadow-lg shadow-green-500/50' : ($onboardingSteps['first_deployment'] ? 'bg-gray-200 dark:bg-slate-700 group-hover:bg-indigo-500 group-hover:shadow-lg group-hover:shadow-indigo-500/50' : 'bg-gray-100 dark:bg-slate-800') }} transition-all">
                        @if($onboardingSteps['setup_domain'])
                            <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                        @else
                            <span class="text-sm font-bold {{ $onboardingSteps['first_deployment'] ? 'text-gray-600 dark:text-slate-300 group-hover:text-white' : 'text-gray-400 dark:text-slate-600' }}">4</span>
                        @endif
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="font-medium text-gray-900 dark:text-white">Add Domain</p>
                        <p class="text-xs text-gray-500 dark:text-slate-400 truncate">SSL & DNS setup</p>
                    </div>
                </a>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        {{-- Cache Performance --}}
        <livewire:dashboard.dashboard-cache-stats />
    </div>

    {{-- Deployment Timeline --}}
    @if(count($deploymentTimeline) > 0 && collect($deploymentTimeline)->sum('total') > 0)
    <div class="bg-white dark:bg-slate-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-slate-700/50 overflow-hidden shadow-sm dark:shadow-none">
        ...
        <div class="p-6">
            <div class="flex items-end gap-2 h-32">
                ...
                <div wire:key="timeline-day-{{ $index }}" class="flex-1 flex flex-col items-center gap-1">
                    ...
                    <span class="text-xs text-gray-500 dark:text-slate-400">{{ $day['date'] }}</span>
                </div>
                @endforeach
            </div>
        </div>
